...
- `knowledge_stage`、`feedback_status` 和 `iteration_fields`：可选的知识迭代字段；一旦填写，必须使用上述枚举和列表格式。处于 `used` 或 `reviewed` 的知识必须填写非空 `used_in`；`reviewed` 必须使用 `feedback_status: recorded`。
