...
| 已消化输入最低标准 | `test_inbox_lifecycle_validates_completed_stages_but_allows_early_stages` 覆盖早期阶段、不完整 digested 与合法完整流转；list-valued stage 被稳定报告而不崩溃 | 通过 |
| 待消化阶段收敛 | `test_backlog_includes_only_pending_inbox_stages` 证明仅缺少阶段、captured、triaged 进入积压与 Top 3；非法 list-valued stage 被排除 | 通过 |
| 读取侧不越界 | inbox、project、formal framework 与周复盘 junction 回归证明外部内容不进入 validator 或 Dashboard | 通过 |
| 周复盘落盘契约 | 真实模板按 `00-系统/每周复盘/YYYY-Www.md` 实例化并被采集；Index 与文件名/week 不一致的记录均被拒绝 | 通过 |
...
- 自动化测试：`python -B -m unittest discover -s tests -v`，71 项通过，0 项失败；使用 `-B`，未产生 `__pycache__` 残留。
...
- 当前正式六框架笔记为 12 篇（项目 1、资产 8、辅助 3）；Git 已跟踪的根目录有 63 篇 Markdown（不含 `AGENTS.md`），其中首轮审计范围剩余 43 篇、首轮后新增 20 篇。未跟踪的新笔记不计入此可复现基线。
